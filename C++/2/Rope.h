...
private:
	...
	void calcSize(); // recalculate size
	int sizeOf(Node* r); // recursive to calculate size
	Node* ithNode(int i); // find the begin point of index i+1
	int calcWeight(Node* r); // recursive calculate weight
	bool is_left(Node* n); // is the left
	bool is_right(Node* n); // is the right
	Node* left_leaf(Node* n); // left leaf
	Node* right_side_leaf(); // right leaf
	Node* nearest_parent(Node* n1, Node* n2); // common parent
private:
	int size_;
	Node *root;
};
